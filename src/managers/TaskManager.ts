...
class BulkTasksManager {
  static get DEFAULTS() {
    return {
      EXPORT_NAMING_CONVENTION:
        ...
        (game.settings?.get(
          moduleId,
          "defaultExportNamingConvention",
        ) as string) || "{foundry}",
      EXPORT_FILE_NAME:
        // @ts-expect-error
        (game.settings?.get(moduleId, "defaultExportZipName") as string) ||
        "bulk-tasks-export",
      DUPLICATE_NAMING_CONVENTION:
        // @ts-expect-error
        (game.settings?.get(
          moduleId,
          "defaultDuplicateNamingConvention",
        ) as string) || "{name} #{index}",
      RENAME_NAMING_CONVENTION:
        // @ts-expect-error
        (game.settings?.get(
          moduleId,
          "defaultRenameNamingConvention",
        ) as string) || "{name}",
      KEEP_IDS_ON_IMPORT:
        // @ts-expect-error
  ...
  static async duplicateDocuments(ids: Set<string>, options: DuplicateOptions) {
    ...
    const namingConvention =
      options.namingConvention || this.DEFAULTS.DUPLICATE_NAMING_CONVENTION;
  ...
  static async exportDocumentsZipFileAsync(
    ...
    if (folderKeys.length > 1 && options.preserveFolders) {
      ...
      const dir: Record<
        string,
        { name: string; entries: any[]; folders: any[] }
      > = {};
  ...
  static async importDocuments(documents: any[], options: ImportOptions) {
    ...
    const operation = options.keepIdsOnImport
  ...
  static async renameDocuments(ids: Set<string>, options: RenameOptions) {
    ...
    // Prepare string args
    const namingConvention =
      options.namingConvention || this.DEFAULTS.RENAME_NAMING_CONVENTION;
    const now = new Date(Date.now()).toString();
    // @ts-expect-error
    await TextEditor._primeCompendiums([{ textContent: namingConvention }]);
  ...
  static async #enrichString(
    ...
    namingConvention = namingConvention
      ...
      .replaceAll(
        "{index}",
        `${options.idx !== undefined ? options.idx + 1 : ""}`,
      )
      .replace("{time}", options.time || "");

    // @ts-expect-error
    const documentTypes = CONST.DOCUMENT_LINK_TYPES.concat([
      "Compendium",
      "UUID",
    ]);
    ...
    const enrichedString = await replaceAsync(
      namingConvention,
      rgx,
      async (match) => {
